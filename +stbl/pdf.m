function p = pdf(x,alpha,beta,gam,delta,varargin)
%STBL.PDF   Pdf of alpha-stable distribution
%   P = STBL.PDF(X,ALPHA,BETA,GAM,DELTA) returns the pdf of the stable
%   distribtuion with characteristic exponent ALPHA, skewness BETA, scale
%   parameter GAM, and location parameter DELTA, at the values in X.  We
%   use the parameterization of stable distribtuions used in [2] - The
%   characteristic function phi(t) of a S(ALPHA,BETA,GAM,DELTA) random
...
%
%   P = STBL.PDF(X,ALPHA,BETA,GAM,DELTA,TOL) computes the pdf to within an
...
%
%   P = STBL.PDF(...,'quick') skips the step of locating the peak in the
...
%   STBL.PDF also excepts a logical true or false (for quick or not quick)
%
%   See also: STBL.RND, STBL.CDF, STBL.INV, STBL.FIT
%
%   References:
%   [1] J. P. Nolan (1997)
...
if nargin < 5
   error('stbl:pdf:TooFewInputs','Requires at least five input arguments.');
end

% Check parameters
if alpha <= 0 || alpha > 2 || ~isscalar(alpha)
   error('stbl:pdf:BadInputs',' "alpha" must be a scalar which lies in the interval (0,2]');
end
if abs(beta) > 1 || ~isscalar(beta)
   error('stbl:pdf:BadInputs',' "beta" must be a scalar which lies in the interval [-1,1]');
end
if gam < 0 || ~isscalar(gam)
   error('stbl:pdf:BadInputs',' "gam" must be a non-negative scalar');
end
if ~isscalar(delta)
   error('stbl:pdf:BadInputs',' "delta" must be a scalar');
end

% Warn if alpha is very close to 1 or 0
if ( 1e-5 < abs(1 - alpha) && abs(1 - alpha) < .02) || alpha < .02
   warning('stbl:pdf:ScaryAlpha',...
...
elseif abs(alpha - 1) > 1e-5          % Gen. Case, alpha ~= 1
   ...
   if any(x(:) > zeta)
      ...
      elseif ~quick % Locate peak in integrand and split up integral
         ...
         p(x > zeta) = c2 .* ...
            (theta2shift1 .* integral(@(theta) zexpz( g1(theta) ),...
            0 , .5, 'AbsTol', tol,'ArrayValued',true) ...
            + theta2shift2 .* integral(@(theta) zexpz( g2(theta) ),...
            .5 , 1, 'AbsTol', tol,'ArrayValued',true) );
         ...
         p( x > zeta ) = c2 .* integral(@(theta) zexpz( g(theta) ),...
            -theta0 , pi/2, 'AbsTol', tol,'ArrayValued',true);
      end
   ...
   if any(x(:) < zeta)
      p( x < zeta ) = stbl.pdf( -xold( x<zeta ),alpha,-beta,...
         gam , -delta , tol , quick);
   end
   ...
   if ~quick  % Locate peak in integrand and split up integral
      ...
      p = c2 .* ...
         (theta2shift1 .* integral(@(theta) zexpz( logg1(theta) ),...
         0 , .5, 'AbsTol', tol,'ArrayValued',true) ...
         + theta2shift2 .* integral(@(theta) zexpz( logg2(theta) ),...
         .5 , 1, 'AbsTol', tol,'ArrayValued',true) );
      ...
      logg = @(theta) xterm + logV(theta);
      zexpz = @(z) max(0,exp(z) .* exp(-exp(z))); % use max incase of NaN
      p = c2 .* integral(@(theta) zexpz( logg(theta) ),-theta0 , pi/2, ...
         'AbsTol', tol, 'ArrayValued', true);
   end
...
% Solves equation g(theta) - 1 = 0 in STBL.PDF using a vectorized bisection
